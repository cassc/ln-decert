...
contract UniswapV3BasicTest is Test {
    ...
    function testCreatePool() public {
        (address token0, address token1) = sortTokens(address(dai), address(usdc));

        address pool = factory.createPool(token0, token1, FEE_MEDIUM);

        assertTrue(pool != address(0), "Pool should be created");
        assertEq(factory.getPool(token0, token1, FEE_MEDIUM), pool);
        assertEq(factory.getPool(token1, token0, FEE_MEDIUM), pool);
    }
    ...
    function testInitializePool() public {
        ...
        assertEq(uint256(sqrtPriceX96), uint256(SQRT_PRICE_1_1), "Price should be initialized");
    }
    ...
    function testEnableNewFeeAmount() public {
        ...
        // Owner 可以启用
        factory.enableFeeAmount(newFee, newTickSpacing);
        assertEq(int256(factory.feeAmountTickSpacing(newFee)), int256(newTickSpacing));
    ...
    function testPoolParameters() public {
        ...
        // 验证 tick spacing
        assertEq(int256(pool.tickSpacing()), int256(TICK_SPACING_MEDIUM), "Tick spacing should match");
