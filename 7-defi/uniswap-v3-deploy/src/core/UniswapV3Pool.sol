...
contract UniswapV3Pool is IUniswapV3Pool, NoDelegateCall {
    ...
    struct Slot0 {
        ...
        // 下一次观察数组扩容后的最大容量，在 observations.write 中触发
        uint16 observationCardinalityNext;
        // 协议费用的分母（1/x）形式，即从交换费中收取协议费的比例
        uint8 feeProtocol;
        // 池是否解锁状态（用于重入保护）
        bool unlocked;
    }
    ...
    /// @dev 函数间的互斥重入保护，同时防止在池未初始化时调用函数
    modifier lock() {
    ...
    /// @dev 返回区块时间戳截断至 32 位（mod 2**32）。该方法在测试中被重写。
    function _blockTimestamp() internal view virtual returns (uint32) {
        return uint32(block.timestamp); // truncation is desired
    }

    /// @dev 获取池中 token0 的余额
    /// @dev 此函数针对 gas 进行了优化，避免了不必要的 extcodesize 检查
    ...
    /// @dev 修改一个仓位的流动性
    /// @param params 仓位参数，包含仓位信息和要应用的流动性增减量
    /// @return position 存储指针，指向给定所有者和 Tick 范围的仓位
    /// @return amount0 池需要支付或收取的 token0 数量（负数表示支出）
    /// @return amount1 池需要支付或收取的 token1 数量（负数表示支出）
        ...
        returns (Position.Info storage position, int256 amount0, int256 amount1)
    {
        ...
        if (params.liquidityDelta != 0) {
            if (_slot0.tick < params.tickLower) {
                // 当前价格在范围下方，流动性只能通过从左向右跨越该范围来增加
                // 因此用户需要提供更多 token0（该方向上价格更高）
                ...
                // 当前价格在范围上方，流动性只能通过从右向左跨越该范围来增加
                // 因此用户需要提供更多 token1（该方向上价格更高）
                amount1 = SqrtPriceMath.getAmount1Delta(
                    TickMath.getSqrtRatioAtTick(params.tickLower),
                    TickMath.getSqrtRatioAtTick(params.tickUpper),
                    params.liquidityDelta
    ...
    /// @dev 获取并更新具有给定流动性增量的仓位
    /// @param owner 仓位的所有者
    /// @param tickLower 仓位范围的下 Tick
    /// @param tickUpper 仓位范围的上 Tick
    /// @param liquidityDelta 流动性增减量
    /// @param tick 当前 Tick，传入以避免重复读取存储
