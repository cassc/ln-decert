...
interface IUniswapV3PoolActions {
    ...
    /// 参数 sqrtPriceX96 池的初始 sqrt 价格为 Q64.96
    ...
    /// 参数 接收者 将为其创建流动性的地址
    /// 参数 tickLower 添加流动性的头寸的下限价位
    /// 参数 tickUpper 添加流动性的头寸的上限
    /// 参数 amount 铸造的流动性数量
    /// 参数 data 应传递给回调的任何数据
    /// 返回 amount0 为铸造给定量的流动性而支付的 token0 的数量。匹配回调中的值
    /// 返回 amount1 为铸造给定量的流动性而支付的 token1 的数量。匹配回调中的值
    function mint(
        address recipient,
        int24 tickLower,
        int24 tickUpper,
    ...
    /// 参数 接收者 应接收所收取费用的地址
    /// 参数 tickLower 收取费用的仓位下限价位
    /// 参数 tickUpper 收费仓位的上限
    /// 参数 amount0Requested 应从所欠费用中提取多少 token0
    /// 参数 amount1Requested 应从所欠费用中提取多少 token1
    ...
    /// 参数 tickLower 消耗流动性的头寸的下限
    /// 参数 tickUpper 消耗流动性的头寸的上限
    /// 参数 amount 要燃烧多少流动性
    /// 返回 amount0 发送给接收者的 token0 数量
    /// 返回 amount1 发送给接收者的 token1 数量
    function burn(
        int24 tickLower,
        int24 tickUpper,
        uint128 amount
    ...
    /// 参数 接收者 接收交换输出的地址
    /// 参数 ZeroForOne 交换方向，true表示token0到token1，false表示token1到token0
    /// 参数 amountSpecified 交换金额，隐式将交换配置为精确输入（正）或精确输出（负）
    /// 参数 sqrtPriceLimitX96 Q64.96 sqrt 价格限制。如果以零换一，价格不能低于这个
    /// 交换后的值。如果一换零，互换后价格不能大于这个值
    /// 参数 data 要传递给回调的任何数据
    /// 返回 amount0 池子 token0 余额的 delta，负数时精确，正数时最小
    /// 返回 amount1 池中token1余额的delta，负数时精确，正数时最小
    function swap(
        address recipient,
        bool zeroForOne,
    ...
    /// 参数 接收者 将接收 token0 和 token1 金额的地址
    /// 参数 amount0 要发送的 token0 的数量
    /// 参数 amount1 要发送的 token1 的数量
    /// 参数 data 要传递给回调的任何数据
    function flash(
        address recipient,
        uint256 amount0,
    ...
    /// 输入观察CardinalityNext。
    /// 参数 ObservationCardinalityNext 池要存储的所需最小观测值数量
