...
/// @title Uniswap V3 Factory 的界面
/// @notice Uniswap V3 Factory 有助于创建 Uniswap V3 池并控制协议费用
interface IUniswapV3Factory {
    /// @notice 工厂所有者变更时发出
    /// @param oldOwner 所有者更改之前的所有者
    /// @param newOwner 所有者变更后的所有者
    ...
    /// @param tickSpacing 初始化刻度之间的最小刻度数
    /// @param pool 创建的池的地址
    event PoolCreated(
    ...
    /// @param 费用 启用的费用，以百分之一 BIP 计价
    /// @param tickSpacing 使用给定费用创建的池的初始化刻度之间的最小刻度数
    ...
    /// @param 费用 启用的费用，以百分之一 BIP 计价。如果未启用费用，则返回 0
    /// @return 刻度间距
    ...
    /// @param 费用 启用的费用金额，以百分之一 BIP 计价（即 1e-6）
    /// @param tickSpacing 对于使用给定费用金额创建的所有池强制执行的刻度之间的间距
