...
/// @title IUniswapV3PoolActions#swap 回调
...
interface IUniswapV3SwapCallback {
    ...
    /// @param amount0Delta 池已发送（负）或必须接收（正）的 token0 数量
    /// 交换结束。如果为正，则回调必须将一定数量的 token0 发送到池中。
    /// @param amount1Delta 池已发送（负）或必须接收（正）的 token1 数量
    /// 交换结束。如果为正，则回调必须将相应数量的 token1 发送到池中。
    /// @param data 调用者通过 IUniswapV3PoolActions#swap 调用传递的任何数据
