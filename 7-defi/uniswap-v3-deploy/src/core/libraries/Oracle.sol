...
library Oracle {
    ...
    /// 参数 self 存储的oracle数组
    /// 参数 time oracle初始化的时间，通过block.timestamp截断为uint32
    /// 返回 基数 oracle 数组中填充元素的数量
    /// 返回 cardinalityNext oracle 数组的新长度，与人口无关
    ...
    /// 参数 self 存储的oracle数组
    /// 参数 index 最近写入观测值数组的观测值的索引
    /// 参数 blockTimestamp 新观察的时间戳
    /// 参数 刻度 新观察时的活动刻度
    /// 参数 流动性 新观察时范围内的总流动性
    /// 参数 基数 oracle 数组中填充元素的数量
    /// 参数 cardinalityNext oracle 数组的新长度，与人口无关
    /// 返回 indexUpdated oracle 数组中最近写入的元素的新索引
    /// 返回 cardinalityUpdated oracle 数组的新基数
    function write(
    ...
    /// 参数 self 存储的oracle数组
    /// 参数 current oracle 数组当前的下一个基数
    /// 参数 next 建议的下一个基数将填充到 oracle 数组中
    /// 返回 next 将填充到 oracle 数组中的下一个基数
    function grow(
        Observation[65535] storage self,
        uint16 current,
        uint16 next
    ...
    /// 参数 time 时间戳被截断为 32 位
    /// 参数 a 比较时间戳，从中确定“时间”的相对位置
    /// 参数 b 从中确定`time`的相对位置
    /// 返回 bool 按时间顺序 `a` 是否 <= `b`
    function lte(
        uint32 time,
        uint32 a,
        uint32 b
    ...
    /// 参数 index 最近写入观测值数组的观测值的索引
    /// 参数 基数 oracle 数组中填充元素的数量
    /// 返回 beforeOrAt 在目标之前或处记录的观察结果
    /// 返回 atOrAfter 在目标处或之后记录的观察结果
    ...
    /// 参数 流动性 调用时的总流动性
    /// 参数 基数 oracle 数组中填充元素的数量
    /// 返回 beforeOrAt 在给定时间戳或之前发生的观察
    /// 返回 atOrAfter 在给定时间戳或之后发生的观察
    ...
    /// 参数 self 存储的oracle数组
    /// 参数 time 当前区块时间戳
    /// 参数 timesAgo 回顾的时间量（以秒为单位），此时返回观察结果
    /// 参数 勾号 当前勾号
    /// 参数 index 最近写入观测值数组的观测值的索引
    /// 参数 流动性 当前池内流动性
    /// 参数 基数 oracle 数组中填充元素的数量
    /// 返回 tickCumulative 自池首次初始化以来经过的tick *时间，截至“secondsAgo”
    /// 返回 SecondsPerLiquidityCumulativeX128 自池首次初始化以来经过的时间/ max(1, 流动性)，截至“secondsAgo”
    ...
    /// 参数 self 存储的oracle数组
    /// 参数 time 当前区块.timestamp
    /// 参数 timesAgos 每次回顾的时间量（以秒为单位），此时返回观察结果
    /// 参数 勾号 当前勾号
    /// 参数 index 最近写入观测值数组的观测值的索引
    /// 参数 流动性 当前池内流动性
    /// 参数 基数 oracle 数组中填充元素的数量
    /// 返回 tickCumulatives 自池首次初始化以来经过的tick *时间，截至每个“secondsAgo”
    /// 返回 SecondsPerLiquidityCumulativeX128s 自池首次初始化以来，截至每个“SecondsAgo”的累计秒数 / max(1, 流动性)
    function observe(
        Observation[65535] storage self,
        uint32 time,
