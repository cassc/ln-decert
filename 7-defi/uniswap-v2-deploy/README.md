...
#### 闪电兑换（Flash Swap）使用说明

- 直接对交易对合约调用 `swap`，并传入非空 `data`；Router02 并未封装此能力。
- 接收方合约需实现 `IUniswapV2Callee.uniswapV2Call`，在回调内完成自定义逻辑（套利、清算等）。
- 回调结束前必须将借出的资产与手续费一起归还，手续费可用公式 `amount * 3 / 997 + 1` 或 `getAmountsIn` 计算。
- 归还的资产可以是借出代币，也可以按路径换成另一种代币，只要满足恒定乘积约束即可。
- `amount0Out` 与 `amount1Out` 可以同时为正，表示一次性借出两种代币；只要归还时让手续费调整后的乘积不小于原始 `k` 值即可（常见写法为「本金 + 0.3% 手续费」）。
