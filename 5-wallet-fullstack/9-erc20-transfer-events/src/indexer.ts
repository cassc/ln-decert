  ...
  const syncOnce = async () => {
    ...
    try {
      ...
      if (fromBlock > latestBlock) {
        console.log(
          `No new blocks yet. Next sync from ${fromBlock}, latest on chain ${latestBlock}`
        );
      }
      ...
      while (fromBlock <= latestBlock) {
        ...
        console.log(
          `Syncing blocks ${fromBlock} -> ${toBlock} for token ${options.tokenAddress}`
        );
        ...
        if (transfers.length) {
          console.log(
            `Stored ${transfers.length} transfer(s) from blocks ${fromBlock} -> ${toBlock}`
          );
        } else {
          console.log(`No transfers in blocks ${fromBlock} -> ${toBlock}`);
        }
