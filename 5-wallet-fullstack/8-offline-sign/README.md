...
3. Deploy with `forge create`:
   ...
   ```bash
   forge create --broadcast src/Bank.sol:Bank \
     --rpc-url "$RPC_URL" \
     --private-key "$PRIVATE_KEY" \
     --constructor-args 0x...WETH_ADDRESS \

   # to deploy on Sepolia, use 0xdd13E55209Fd76AfE204dBda4007C227904f0a81
   ```
...
4. Verify the contract on Etherscan:
   ```bash
    ...
    0xDeployedContractAddress \
     src/Bank.sol:Bank \
       --constructor-args $(cast abi-encode "constructor(address)" 0x..WETH_ADDRESS) \
